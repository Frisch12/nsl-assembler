...
public abstract class Statement
{
  ...
  public static AssembleExpression matchInstruction(int returns)
  {
    ...
    if(ScriptParser.tokenizer.match(AddIncludeDirInstruction.name))
      return new AddIncludeDirInstruction(returns);
    if(ScriptParser.tokenizer.match(AddPluginDirInstruction.name))
      return new AddPluginDirInstruction(returns);
    if(ScriptParser.tokenizer.match(IncludeInstruction.name))
      return new IncludeInstruction(returns);
    if(ScriptParser.tokenizer.match(CallInstruction.name))
      return new CallInstruction(returns);
    return null;
  }
