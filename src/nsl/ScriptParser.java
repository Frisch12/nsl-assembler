...
public class ScriptParser
{
   ...
   */
  public static int parse(String path, String output, boolean noPauseOnError, boolean noMakeNSIS, String nsisPath) throws IOException
  {
    int exitCode = 0;
    scriptPath = path;
    ...
    if (statement != null)
    {
      File outputFile = new File(getOutputPath(path, output));
      ...
      if (exitCode == 0)
      {
        ...
        if (!noMakeNSIS)
        {
          File makensisw = new File(nsisPath);
          if (makensisw.exists())
          {
   ...
   */
  private static String getOutputPath(String scriptPath, String output)
  {
    int i = scriptPath.lastIndexOf('.');
    if (i == -1)
      return scriptPath + ".nsi";
    String s = scriptPath.substring(0, i) + ".nsi";
    if(output.endsWith("/")) {
      String[] split = scriptPath.split("/");
      s = output + split[split.length - 1].substring(0, split[split.length - 1].lastIndexOf('.')) + ".nsi";
    } else if(!Objects.equals(output, "")) {
      s = output;
    }
    return s;
  }
