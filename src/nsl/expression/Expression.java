...
public class Expression
{
  ...
  private Expression(Tokenizer tokenizer)
  {
    ...
    {
      if (tokenizer.sval.startsWith("$") || tokenizer.sval.startsWith("@$"))
      {
        String realValue = tokenizer.sval.startsWith("@") ? tokenizer.sval.substring(1) : tokenizer.sval;
        ...
        else
        {
          ...
          else
          {
            ...
            this.stringValue = null;
            this.booleanValue = false;
            this.integerValue = RegisterList.getCurrent().add(realValue);
            ...
            if(tokenizer.sval.startsWith("@")) {
              RegisterList.getCurrent().get(this.integerValue).setRegisterType(RegisterType.Special);
            }
          }
        }
      }
  ...
  private static Expression matchLogicalOrOrBinaryInclusiveOr()
  {
    ...
    while (ScriptParser.tokenizer.match('|'))
    {
      ...
      if (ScriptParser.tokenizer.match('|'))
      {
        if (ExpressionType.isBoolean(left) && left.booleanValue)
        {
  ...
  {
    if (ScriptParser.tokenizer.tokenIsWord() && !ScriptParser.tokenizer.sval.startsWith("$") && !ScriptParser.tokenizer.sval.startsWith("@$") && !ScriptParser.tokenizer.sval.equals("true") && !ScriptParser.tokenizer.sval.equals("false"))
